...
if __name__ == '__main__':
    ...
    log_file = os.path.join(hparams.save_root, 'loggers', hparams.model_name, 'test_videos.txt')
    check_file(log_file)
    ...
    base_name_ = os.path.basename(hparams.content_file).split('.')[0] + "_" + \
                 os.path.basename(hparams.style_file).split('.')[0] + '_' + str(hparams.alpha) + '.mp4'
    #     logger.info(content_)
    #     logger.info(style_)

    cap = cv2.VideoCapture(hparams.content_file)
    fourcc = cv2.VideoWriter_fourcc(*'MP4V')
    out = cv2.VideoWriter(os.path.join(f'{hparams.test_result_root}', base_name_), fourcc, 24.0, (1280, 720))
    while cap.isOpened():
        ret, content_ = cap.read()
        if ret == True:
            frame = cv2.flip(content_, 0)
        # content_ = Image.open(hparams.content_file).convert('RGB')
        # content_ = trans(content_)
        content_ = cv2.cvtColor(content_, cv2.COLOR_BGR2RGB)
        content_ = np.float32(content_).transpose((2, 0, 1))
        content_ = torch.from_numpy(content_ / 255.0)
